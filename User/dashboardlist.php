...
<body>
  ...
  <div id="bscsSubcategories" class="subcategory-list" style="display:none; margin-top: 10px;">
    <a href="dashboard bsais.php"><button onclick="alert('Year 1 clicked')">BSAIS</button></a>
    <button onclick="alert('Year 2 clicked')">BSIT</button>
  </div>
    <div class="user-list">
      <?php
$result = $mysqli->query("SELECT id, firstname, lastname, mi, usn FROM userdata");
if ($result && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // Fetch grades
        $grades = [];
        $gradeResult = $mysqli->query("SELECT type, score, total FROM grades WHERE user_id = $userId");
        ...
        // Fetch coverage
        $coverageRow = $mysqli->query("SELECT * FROM grade_weights WHERE user_id = $userId")->fetch_assoc();
        $coverage = [
            'assignment' => $coverageRow['assignment_weight'] ?? 25,
            'quiz' => $coverageRow['quiz_weight'] ?? 25,
            'activity' => $coverageRow['activity_weight'] ?? 20,
            'exam' => $coverageRow['exam_weight'] ?? 30
        ];
...
echo '<span>' . $counter . '. ' . "$fname $mi. $lname<br>USN: <a href=\"info/user.php?usn=$usn\" style=\"color: #ffdede;\">$usn</a></span>";
echo '<div class="student-average">' . round($finalAverage, 2) . '%</div>';
