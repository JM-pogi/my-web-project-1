...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['grades'], $_POST['user_id'], $_POST['usn'])) {
    ...
    foreach ($_POST['grades'] as $gradeId => $gradeData) {
        ...
        $stmt = $mysqli->prepare("UPDATE bsaisgrades SET score = ?, total = ? WHERE id = ? AND user_id = ?");
        $stmt->bind_param('iiii', $score, $total, $gradeId, $userId);
        $stmt->execute();
    }

    // Redirect back with success
    header("Location: bsaisinfo.php?usn=" . urlencode($usn) . "&success=1");
    exit;
}
...
// Fetch user
$stmt = $mysqli->prepare("SELECT id, firstname, lastname, mi, usn, Password FROM bsaisuserdata WHERE usn = ?");
...
// Fetch grades
$stmtGrades = $mysqli->prepare("SELECT id, type, score, total FROM bsaisgrades WHERE user_id = ?");
...
<body>
  ...
  <div class="frame">
    ...
    <a href="../dashboard bsais.php" class="back-link">Go Back</a>
