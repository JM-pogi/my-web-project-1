...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_usn'])) {
    ...
    // First, get the user ID from bsaisuserdata
    $userResult = $mysqli->query("SELECT id FROM bsaisuserdata WHERE usn = '$delete_usn'");

    if ($userResult && $userResult->num_rows > 0) {
        $userId = $userResult->fetch_assoc()['id'];

        // Delete related grades first
        $mysqli->query("DELETE FROM bsaisgrades WHERE user_id = $userId");

        // Delete the user
        $delete_query = "DELETE FROM bsaisuserdata WHERE usn = '$delete_usn' LIMIT 1";
...
<body>
  <video autoplay loop muted class="vid">
    <source src="forest.mp4"video/mp4 />
  </video>
  ...
  <div class="frame">
    ...
    <!-- Frame 3: User List -->
    <div class="frame3">
  <button id="bscsBtn" class="listframe"><h1>BSAIS</h1></button>

  <!-- Subcategory list container, initially hidden -->
  <div id="bscsSubcategories" class="subcategory-list" style="display:none; margin-top: 10px; position: absolute; margin-top: 55px;"">
    <a href="dashboard.php"><button onclick="alert('go to BSAIS')">BSCS</button></a>
    <button>BSIT</button>
  </div>
    <div class="user-list">
      <?php
$result = $mysqli->query("SELECT id, firstname, lastname, mi, usn FROM bsaisuserdata");
if ($result && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // Fetch grades
        $grades = [];
        $gradeResult = $mysqli->query("SELECT type, score, total FROM bsaisgrades WHERE user_id = $userId");
        ...
        $finalAverage = 0;
        foreach ($grades as $type => $gradeList) {
            $sumScore = 0;
            $sumTotal = 0;
            ...
            $average = ($sumTotal > 0) ? ($sumScore / $sumTotal) * 100 : 0;
            $weight = $coverage[$type] ?? 0;
            $finalAverage += $average * ($weight / 100);
        }
echo '<div class="student-entry">';
echo '<span>' . $counter . '. ' . "$fname $mi. $lname<br>USN: <a href=\"checkuser/bsaisinfo.php?usn=$usn\" style=\"color: #ffdede;\">$usn</a></span>";
...
echo '</div>';
  ...
  <div id="menuFrame">
    ...
    <form id="studentForm" method="POST" action="bsaissubmit.php">
      ...
      <div class="row" style="margin-top: 20px;">
        <label>Assignment % <input type="number" class="input-field coverage" name="coverage[]" value="10" required /></label>
        <label>Quiz % <input type="number" class="input-field coverage" name="coverage[]" value="10" required /></label>
        <label>Activity % <input type="number" class="input-field coverage" name="coverage[]" value="40" required /></label>
        <label>Exam % <input type="number" class="input-field coverage" name="coverage[]" value="40" required /></label>
      </div>
...
<script>
  ...
  const bscsSubcategories = document.getElementById('bscsSubcategories');
  const userList = document.getElementById('userList');

  bscsBtn.addEventListener('click', () => {
    if (bscsSubcategories.style.display === 'none' || bscsSubcategories.style.display === '') {
      bscsSubcategories.style.display = 'block';
      userList.style.display = 'none';  // optionally hide user list when showing subcategories
    } else {
      bscsSubcategories.style.display = 'none';
      userList.style.display = 'block'; // show user list when hiding subcategories
    }
  });
</script>
