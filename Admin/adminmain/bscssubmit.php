...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $mysqli->prepare("INSERT INTO bscsuserdata (firstname, lastname, mi, usn, Password) VALUES (?, ?, ?, ?, ?)");
    ...
    $stmt = $mysqli->prepare("
        ...
        VALUES (?, ?, ?, ?, ?)
    ");
    $stmt->bind_param("idddd", $userId, $coverage[0], $coverage[1], $coverage[2], $coverage[3]);
    $stmt->execute();
    $stmt->close();

    // ✅ Insert grades
    $types = ['assignment', 'quiz', 'activity', 'exam'];
    $gradeStmt = $mysqli->prepare("INSERT INTO bscsgrades (user_id, type, score, total) VALUES (?, ?, ?, ?)");
