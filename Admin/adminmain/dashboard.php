...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_usn'])) {
    ...
    $delete_query = "DELETE FROM bscsuserdata WHERE usn = '$delete_usn' LIMIT 1";
...
<head>
  ...
  border-radius: 5px;
  box-shadow: 2px 2px 5px rgba(72, 72, 72, 0.1);
  margin-left: 10px;
}
...
<body>
  ...
  <div class="frame">
    ...
    <!-- Frame 3: User List -->
    <div class="frame3">
  <button id="bscsBtn" class="listframe"><h1>BSCS</h1></button>
  ...
  <div id="bscsSubcategories" class="subcategory-list" style="display:none; margin-top: 10px; position: absolute; margin-top: 55px;"">
    <a href="dashboard bsais.php"><button onclick="alert('go to BSAIS')">BSAIS</button></a>
    <button>BSIT</button>
  </div>
    <div class="user-list">
      <?php
$result = $mysqli->query("SELECT id, firstname, lastname, mi, usn FROM bscsuserdata");
if ($result && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // Fetch grades
        $grades = [];
        $gradeResult = $mysqli->query("SELECT type, score, total FROM bscsgrades WHERE user_id = $userId");
...
echo '<span>' . $counter . '. ' . "$fname $mi. $lname<br>USN: <a href=\"checkuser/bscsinfo.php?usn=$usn\" style=\"color: #ffdede;\">$usn</a></span>";
...
echo '</div>';
  ...
  <div id="menuFrame">
    ...
    <form id="studentForm" method="POST" action="bscssubmit.php">
